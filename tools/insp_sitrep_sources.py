...
def _classify_pdf_structure(parsed: dict[str, str]) -> None:
    ...
    table_extracted = table_status == "table_ii_extracted"
...
def _extract_aligned_cumulative_banner(text: str) -> dict[str, str]:
    ...
    has_split_suspected = "suspected_cases_under_investigation" in extracted and (
        "suspected_cases_in_isolation" in extracted
        or "patients_in_isolation_hospitalization" in extracted
    )
    ...
    for line_idx, line in enumerate(lines[:80]):
        ...
        for match in re.finditer(r"Patients?", line, re.I):
            ...
            if "isolement" in context or "hospitalisation" in context:
                anchors.setdefault("patients_in_isolation_hospitalization", (line_idx, match.start()))
...
def table_ii_status(text: str, parsed: dict[str, str] | None = None) -> str:
    ...
    return "table_ii_extracted" if extract_table_ii(text, source) else "table_ii_unparsed"
...
def _case_distribution_table_start(text: str) -> int:
    ...
    for start in reversed(starts):
        ...
        if "zones de santé" not in compact_window and "zone de santé" not in compact_window:
            continue
        if "Nbre de cas" not in compact_window and "cas suspects" not in compact_window:
            continue
...
def write_source_reports(path: Path, sources: Iterable[ReportSource]) -> None:
    ...
    columns = [
        ...
        "national_patients_in_isolation_hospitalization",
        "national_recovered",
        "draft_status",
        "blocking_reason",
        "revision_notes",
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for source in sources:
            row = {
                ...
                "national_patients_in_isolation_hospitalization": source.parsed.get(
                    "national_patients_in_isolation_hospitalization", ""
                ),
...
def write_processed_candidates(processed_dir: Path, draft_values: Iterable[ProcessedDraftValue]) -> list[Path]:
    ...
    blocked_keys = {
        (issue["metric"], issue["nom_key"], _date_key(issue["date"]))
        for issue in _cumulative_review_issues(processed_dir, draft_list)
    ...
    for value in draft_list:
        if value.metric not in PROCESSED_DRAFT_BY_METRIC:
            continue
        if (value.metric, _nom_key(value.nom), _date_key(value.date)) in blocked_keys:
            continue
