...
def test_parse_report_text_extracts_sitrep18_patient_isolation_field():
    ...
    assert parsed["national_patients_in_isolation_hospitalization"] == "173"
...
def test_processed_review_blocks_candidate_confirmed_cumulative_decrease(tmp_path: Path):
    processed = tmp_path / "processed"
    processed.mkdir()
    path = processed / "insp_sitrep__national_cumulative_confirmed_cases__daily.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    assert row["draft_status"] == "blocked"
    assert row["blocking_reason"] == "cumulative_decrease:2026-05-30=321>2026-05-31=300"


def test_processed_candidates_skip_confirmed_cumulative_decrease(tmp_path: Path):
    processed = tmp_path / "processed"
    processed.mkdir()
    path = processed / "insp_sitrep__national_cumulative_confirmed_cases__daily.csv"
    path.write_text(
    ...
    assert sources.write_processed_candidates(processed, drafts) == []
...
def test_processed_candidates_skip_suspected_cumulative_decrease(tmp_path: Path):
    processed = tmp_path / "processed"
    processed.mkdir()
...
def test_processed_candidates_skip_alias_equivalent_cumulative_decrease(tmp_path: Path):
    processed = tmp_path / "processed"
    processed.mkdir()
...
def test_processed_candidates_skip_candidate_that_creates_future_decrease(tmp_path: Path):
    processed = tmp_path / "processed"
    processed.mkdir()
    path = processed / "insp_sitrep__national_cumulative_confirmed_cases__daily.csv"
    path.write_text(
